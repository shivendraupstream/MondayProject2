...
- mapping-schema is implemented but not confirmed working end-to-end. routes/mapping.js correctly builds a schema from the selected template's active version test data and returns it in the shape monday's dynamic mapping fields expect, but monday has not been observed successfully retrieving/populating that schema in the recipe builder UI. The route itself responds correctly when hit directly (curl/Postman), so the gap is somewhere between monday calling this URL as a dynamic mapping Field Definitions endpoint and it actually rendering the fields, not confirmed whether it's a payload/dependency issue on monday's side, a configuration issue in the Developer Center block setup, or something else. Unresolved as of now, worth revisiting with the request logs already present in that route (*** mapping-schema route HIT ***) the next time it's tested live.
